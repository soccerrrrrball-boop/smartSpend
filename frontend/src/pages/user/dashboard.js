...
function Dashboard() {
    ...
    return (
        <Container activeNavId={0}>
            ...
            {(!isError && !isLoading) && <SelectMonth months={months} onMonthChange={onMonthChange} />}
            {(!isError && !isLoading && total_expense === 0 && total_income === 0 && no_of_transactions === 0) && <Info text={"You have no transactions in this month!"} />}
            {
                (!isError && !isLoading && (total_expense !== 0 || total_income !== 0 || no_of_transactions !== 0)) && <>
